#!/usr/bin/env bash
# Paczka v4: niezależny recheck answerability flag pełnego audytu + próbka kontrolna.
#
#   scripts/run_night_jobs_v4.sh <BASE_URL> <MODEL> <API_KEY> [CONCURRENCY]
set -euo pipefail
...
IN="artifacts/task06/night_jobs_v4/input"
OUT="artifacts/task06/night_jobs_v4/verdicts"
[ -d "$IN" ] || { echo "brak $IN — rozpakuj night_jobs_v4_input.tar.gz w katalogu repo" >&2; exit 2; }
