...
if ! flock -n 9; then
  echo "another D01b probe phase owns the campaign lock" >&2
  exit 3
fi
if [[ ! -x $PYTHON ]]; then
...
assert_gpu_idle() {
  ...
  if [[ $query_rc -ne 0 ]]; then
    ...
    if [[ $pmon_rc -ne 0 ]]; then
      echo "cannot verify GPU compute-process state; refusing D01b probe training" >&2
      return 5
    fi
  fi
  if [[ -n $active ]]; then
    echo "GPU has active compute processes; refusing D01b probe training" >&2
    echo "$active" >&2
    return 4
  fi
}
...
preflight() {
  "$PYTHON" scripts/run_d01b_probe_dev_screen.py preflight --config "$CONFIG"
}
...
compare() {
  "$PYTHON" scripts/run_d01b_probe_dev_screen.py compare --config "$CONFIG"
}
