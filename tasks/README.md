...
| [05](05_controlled_diversity_and_multiquery.md) | Kontrolowany styl, focus i multi-query | `IMPLEMENTED` | D01 1.5B/4.5B i retrospektywna diagnostyka D01b są ukończone, ale nie autoryzują probe. Prospective v1 zatrzymało się fail-closed przed scoringiem z powodu 320 grup W05 bez exact K. Po akceptacji właściciela zamrożono prospective v2 przed generacją: nowa kohorta 2000 rekordów nie przecina rank-10 ani v1, minimum 5 hard negative’ów, SHA-256 seed 20260803, niezmieniony selector `2164822`, sędziowie i bramki, z technicznym limitem 16 prób na slot; `final_tests_used=[]`. Runner v2 i testy są gotowe; CPU `preflight` i idempotentne `prepare-cohort` przeszły, generacji nie uruchomiono. Następny krok to osobno autoryzowane `generate`, a po exact K fazy `score` i `select-compare`. Probe, 4.5B i finalne testy pozostają zamknięte. Groq proxy ukończył 500/500 etykiet i 200/200 audytów; agreement człowieka pozostaje `NOT MEASURED`, a D00–D12 poza D01 nie są zmierzone. |
