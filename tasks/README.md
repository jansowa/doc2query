...
| [03](03_sft_qlora_baselines.md) | Baseline'y SFT/QLoRA | `IMPLEMENTED` | S00 jest zmierzone na frozen dev: oba raporty mają po 25k completionów; few-shot traci względem zero-shot w corpus RT@20 (`0,3824` vs `0,6492`) i formacie (`0,0376` vs `0,1507`). ADR daje `GO` dla przygotowania S07 i `HOLD` dla P-06 do zamrożenia planu S07 oraz dostępności marginów train. `dev_confirm` i testy finalne pozostają zamknięte. |
...
2. **Następna kolejka baseline:** S00 jest kompletne i zmierzone na frozen
   dev. Osobny ADR S07/P-06 daje `GO` dla przygotowania prospektywnego
   kontraktu S07 oraz `HOLD` dla P-06 do zamrożenia tego planu i dostępności
   offline marginów train. Nie zmieniać progu P-04 po obejrzeniu wyników.
...
Najbliższy punkt wejścia to przygotowanie prospektywnego kontraktu S07 z
budżetem porównywalnym do Bielika 1.5B/W05.
...
S07 wymagane i niewykonane, P-06 wstrzymane do spełnienia zależności, a
