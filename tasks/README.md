...
| [05](05_controlled_diversity_and_multiquery.md) | Kontrolowany styl, focus i multi-query | `IMPLEMENTED` | D01 1.5B/4.5B i retrospektywna diagnostyka D01b są ukończone, ale nie autoryzują probe. Prospective v1 i v2 zatrzymały się fail-closed przed scoringiem przez niepełne distinct exact K W05; v2 miało 7912/8000 query i 62 niepełne grupy po 16 próbach. Po akceptacji właściciela zamrożono prospective v3 przed generacją: świeża kohorta 2000 nie przecina rank-10/v1/v2, seed 20260804, W05 zachowuje cztery format-valid sloty wraz z duplikatami, controlled i selector `2164822` pozostają bez zmian, dodano duplicate-rate guardrail; `final_tests_used=[]`. Runner v3 jest gotowy; CPU `preflight` i idempotentne `prepare-cohort` przeszły, 254 testy/Ruff/mypy są czyste, generacji nie uruchomiono. Następny krok to osobno autoryzowane `generate`, następnie `score` i `select-compare`. Probe, 4.5B i finalne testy pozostają zamknięte. Groq proxy ukończył 500/500 etykiet i 200/200 audytów; agreement człowieka pozostaje `NOT MEASURED`, a D00–D12 poza D01 nie są zmierzone. |
