...
| [06](06_candidate_scoring_and_preference_data.md) | Scoring kandydatów i dane preferencyjne | `IN PROGRESS` | Pilot 512×8 ukończony: 4096/4096 scoringów i 2048 safe-selected (1164 W06 + 884 D01), selector zmienił 482/512 grup. Naprawiono wyłącznie błędne etykiety `smoke` w provenance i odbudowano selekcję; teksty/score’y bez zmian. Pilot nie wystarcza do DPO, bo sloty D01 mają różne prompty. Zamrożono więc same-prompt expansion: 500 promptów D01 × 8 odpowiedzi, potem scoring, tentative pairs i dual-LLM Groq. Run expansion uruchomiono odłączony; Task 07 nadal zamknięty, `final_tests_used=[]`. Rozszerzenie 2026-08-13: przed budową par obowiązuje bramka różnorodności same-prompt; dopuszczono prospektywną ablację teachera (lokalny Qwen3.6-27B Q4) i lokalnego sędziego qwen3.6-27b za osobnym ADR. |
...
| [08](08_grpo_multiobjective_rl.md) | Wielokryterialny GRPO/RL | `BLOCKED` | Od 2026-08-13 planowany etap programu (cel edukacyjny właściciela, AGENTS.md §2): ścieżka 1.5B/8 GB po Task 07, R06 4.5B preferencyjnie na 16 GB; wymagana decyzja `reports/decisions/enable_grpo.md`. Porównanie DPO vs continued SFT vs best-of-N jest kontekstem interpretacyjnym; twarde przesłanki Fazy E są bramką awansu do selekcji finalistów, nie startu. |
