...
| [06](06_candidate_scoring_and_preference_data.md) | Scoring kandydatów i dane preferencyjne | `IN PROGRESS` | Gotowe są model-free fundament, planner requestów K=4–8, kontrakty outputu generatora i dowodów scoringowych oraz fail-closed assembler nierankingowanego evidence. Dodany handoff Task 06 → 07 pakuje wyłącznie wcześniej zamrożone preference/continued-SFT train+dev i zewnętrzny artefakt przypisań wag; wymaga pełnego pokrycia/provenance, wykrywa leakage i nie otwiera testu. 23 nowe testy handoffu przechodzą. Generacja, scoring sędziami/retrieverem, kalibracja, wyliczenie i zamrożenie funkcji wag/progów, właściwe preference train/dev oraz audyt człowieka pozostają niewykonane i zamknięte. |
| [07](07_dpo_training.md) | DPO i continued-SFT control | `IN PROGRESS` | Gotowy jest model-free fundament oraz deterministyczny handoff wejść: ścisłe kontrakty, walidacja preference/continued-SFT/weighted-SFT, provenance i hashe, trzyramienny matched-budget planner, walidacja precomputed reference logprobs i toy sigmoid DPO loss. Handoff tworzy dokładnie jeden control na `preference_id`, odmawia nadpisania i publikuje staging atomowo. Łącznie 47 ukierunkowanych lekkich testów handoffu i istniejącego fundamentu DPO przechodzi; nie ładowano modeli ani nie rozpoczynano treningu. Tokenizacja modelem, właściwy precompute logprobów, DPO/controls, modelowe smoke testy, PEFT save/load, ewaluacja, wiele seedów i bramki pozostają niewykonane i zamknięte. |
