...
| [05](05_controlled_diversity_and_multiquery.md) | Kontrolowany styl, focus i multi-query | `IMPLEMENTED` | Gotowe są kontrakty i kod CPU, w tym wznawialny post-D01 pipeline frozen-dev: controlled/matched K=4 generation, primary/shadow scoring, slice'y, paired bootstrap i fail-closed HN0+filter/drop probe-input materialization. Nocna kolejka ma dopasowane treningi 1.5B/4.5B i generation-only, ale nie została uruchomiona; scoring i probe pozostają osobnymi bramkami. D00–D12, audyty 500/200, kalibracja per domena, human check i probe z CI nadal są niewykonane. |
