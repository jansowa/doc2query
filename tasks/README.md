...
| [05](05_controlled_diversity_and_multiquery.md) | Kontrolowany styl, focus i multi-query | `IMPLEMENTED` | Prospective D01b v3 i wszystkie 7 bramek są kompletne; copy-risk spadł z `0.07125` do `0.0465`. Equal-budget probe inputs 1.5B hybrid-vs-W05 mają po 7936 par, 1984 dokumenty i K=4. Batch-4 W05 ukończył trening 500/500, lecz ewaluacja zatrzymała się po wznowieniu: wcześniejsze wyłączenie pozostawiło pusty shard 5/100, a cache sprawdzał tylko obecność pliku. Pozostałe 99 shardów jest poprawnych; hybrid nie wystartował i nie ma wyniku porównania. Dodano atomowy trwały zapis oraz walidację i punktową naprawę shardów; test regresyjny i CPU preflight przechodzą. Następne `run-all` zachowa trening i 99 shardów W05, przeliczy wyłącznie uszkodzony shard, dokończy oba ramiona i compare. `dev_confirm` wymaga wyniku `eligible` i osobnej zgody; 4.5B oraz finalne testy pozostają zamknięte (`final_tests_used=[]`). Groq proxy ukończył 500/500 etykiet i 200/200 audytów; agreement człowieka pozostaje `NOT MEASURED`, a D00–D12 poza D01 nie są zmierzone. |
...
kontraktu zakończył się `rc=0`. Batch-4 W05 ukończył następnie trening, a jego
cache korpusu ma 99 poprawnych shardów i jeden pusty shard po kolejnym
wyłączeniu. Walidacja i punktowa naprawa cache są wdrożone; wznowienie nie
powtórzy treningu ani poprawnych shardów. Najbliższy punkt wejścia to ponowienie
dev-only runu:
