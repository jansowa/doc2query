...
| [05](05_controlled_diversity_and_multiquery.md) | Kontrolowany styl, focus i multi-query | `IMPLEMENTED` | Prospective D01b v3 i wszystkie 7 bramek są kompletne; copy-risk spadł z `0.07125` do `0.0465`. Equal-budget probe inputs 1.5B hybrid-vs-W05 mają po 7936 par, 1984 dokumenty i K=4. Osobny ADR zamroził `dev_screen`: wspólny prefix 1984 par/496 dokumentów, 250 kroków, seed 42, ocena na 6598 naturalnych `dev_intrinsic_rank10`; CPU preflight przeszedł, trening nie wystartował. Jedna komenda operatorska ma ETA około 7–8 h. `dev_confirm` wymaga wyniku `eligible` i osobnej zgody; 4.5B oraz finalne testy pozostają zamknięte (`final_tests_used=[]`). Groq proxy ukończył 500/500 etykiet i 200/200 audytów; agreement człowieka pozostaje `NOT MEASURED`, a D00–D12 poza D01 nie są zmierzone. |
...
Najbliższy punkt wejścia to około 7–8-godzinny dev-only run:
