...
| [06](06_candidate_scoring_and_preference_data.md) | Scoring kandydatów i dane preferencyjne | `IN PROGRESS` | Gotowe są model-free fundament, planner requestów K=4–8 oraz ścisłe kontrakty outputu generatora i dowodów scoringowych. Fail-closed assembler łączy kompletne artefakty 1:1, wykrywa drift/leakage/duplikaty, odrzuca test i zapisuje nierankingowany bundle z manifestem; nie liczy total score i nie uruchamia modeli. Dla nowego przyrostu oraz wcześniejszego planera/selekcji przechodzi ukierunkowany zestaw 22 lekkich testów. Generacja, scoring sędziami/retrieverem, kalibracja, wagi/progi, właściwe preferencje i audyt człowieka pozostają zamknięte do rozstrzygnięcia Task 05 oraz osobnej prospektywnej konfiguracji. |
| [07](07_dpo_training.md) | DPO i continued-SFT control | `IN PROGRESS` | Gotowy jest wyłącznie model-free fundament: ścisłe kontrakty, fail-closed walidacja danych i precomputed reference logprobs, trzyramienny matched-budget planner oraz toy sigmoid DPO loss. Ukierunkowane 24 lekkie testy CPU przechodzą; nie ładowano modeli ani nie rozpoczynano treningu. Właściwy precompute, tokenizacja modelem, DPO/controls, modelowe smoke testy, PEFT save/load, ewaluacja, wiele seedów i bramki pozostają niewykonane i zamknięte. |
